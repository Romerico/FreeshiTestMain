...
//13.Click on Ok.
        ...
        String timeBefore = p.getTimeString();

        p.changeTimeAndConfirm();

        String timeAfter = p.getPickUptime();
//13. User should be able to select the time slot for today from the available time slots.


        softassert.assertFalse(timeBefore.equals(timeAfter),
                "Scheduled time did not change, before : " + timeBefore + ", after : " + timeAfter);


    softassert.assertAll();

        logger.info("pickup time successfully changed, before : " + timeBefore + ", after : " + timeAfter);

        logger.info("completed FWA_071");
